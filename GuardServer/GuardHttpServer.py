...
class GuardHttpServer:
	...
	def uploadOpt(self, sock, upload_content):
		...
		dirPath = "/Users/4dage-imac2/Desktop/testdir" + path
	...
	def downloadOpt(self, sock, path):
		...
		try:
			filePath = "/Users/4dage-imac2/Desktop/testdir" + path
	...
	def echo_showfile(self, path, sock):
		...
		try:
			path = "/Users/4dage-imac2/Desktop/testdir" + path
			file = open(path, "rb")
			name = os.path.basename(path)
	...
	def handle_client(self, sock, server_socket):
		...
		if ret < 0:
			...
			if rtnStr.find(SER_TAG_HTTP) >= 0:
				...
				elif method == SER_TAG_POST and len(path) > len(SER_TAG_UPLOAD) and (path[0:len(SER_TAG_UPLOAD)] == SER_TAG_UPLOAD):
	...
	def __init__(self, name=__name__):
		...
		# try:
		# 	# file = open("/Users/4dage-imac2/Desktop/彭罗榕-10-11月工时及任务记录1.txt", "r")
		# 	# file = open("/Users/4dage-imac2/Desktop/testdir", "r")
		# 	file = open("/Users/4dage-imac2/Desktop/彭罗榕-10-11月工时及任务记录.txt", "r")
		# 	while True:
		# 		log.info(">>>>>>>>>>")
		# 		ret = file.read(1024)
		# 		log.info("read rtn:" + str(ret) + " len:" + str(len(ret)))
		# 		if len(ret) <= 0:
		# 			log.info("read end>>>>>>")
		# 			break
		# except FileNotFoundError:
