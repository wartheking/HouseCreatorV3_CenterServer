...
class Logger:
    ...
    def getLogDirPath(self):
        ...
        pathLogDir = pathDirNow + self.getPathSeperater() + ".." + self.getPathSeperater() + self.getPathSeperater() + ".." + self.getPathSeperater() + self.LOG_DIR_NAME
        return pathLogDir
...
def RUNEND(status=JTAG_PID_STATUS_FINISHED):
    ...
    try:
        ...
        #可能是单独测试，就不管它发送是否成功了
        log.info("RUNEND() error!!! what ever~")
...
#默认方法 end
##############
from CenterHttpServer import *
try:
    RUNEND(JTAG_PID_STATUS_BACKGROUND)
    center = CenterHttpServer(log)
